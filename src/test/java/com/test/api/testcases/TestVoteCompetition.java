...
public class TestVoteCompetition extends Base {
  ...
  public void getQuota() {
    ...
    .headers("Authorization", token.login("6282278843303", "dikakoko", "647464", "android")).log().all().when().get().then().log().all().statusCode(200).log().all();
  ...
  public void postVote() {
    ...
    rs.baseUri(baseUrl.urlUgcVote("/v1/vote/competition")).contentType(ContentType.JSON).headers("Authorization", token.login("6282278843303", "dikakoko", "647464", "android")).body(vote).log().all().then().log().all().statusCode(200).log().all();
  ...
  public void postVoteLebih() {
    ...
    rs.baseUri(baseUrl.urlUgcVote("/v1/vote/competition")).contentType(ContentType.JSON).headers("Authorization", token.login("6282278843303", "dikakoko", "647464", "android")).body(vote).log().all().then().log().all().statusCode(422).log().all();
  ...
  public void postVoteNullQuota() {
    ...
    for (int i = 3; i <= 3; i++) {
    rs.baseUri(baseUrl.urlUgcVote("/v1/vote/competition")).contentType(ContentType.JSON).headers("Authorization", token.login("dikakoko04@gmail.com", "dikakoko", "1234", "android")).body(vote).log().all().then().log().all().statusCode(200).log().all();
    }
