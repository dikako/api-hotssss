...
public class TestComment extends Base {
  ...
  public void postComment() {
    ...
    rs.baseUri(baseUrl.urlComment("/v1/comments/user")).contentType(ContentType.JSON).headers("Authorization", token.login("6282278843303", "dikakoko", "647464", "android")).body(comment).log().all().then().log().all().statusCode(201).log().all();
