...
public class Validation {
  ...
  public void getBody(String path, RequestSpecification rs, String body) {
    Response res = rs.get();
    Assert.assertEquals(res.getBody().path(path).toString().replace(" ", "").toLowerCase().replace("[", "").replace("]", ""),
  ...
  public void getBodyContains(String path, RequestSpecification rs, String body) {
    Response res = rs.get();
    res.getBody().path(path).toString().replace(" ", "").toLowerCase().replace("[", "").replace("]", "").contains(body.replace(" ", "").toLowerCase().replace("[", "").replace("]", ""));
    System.out.println("Response Body: " + res.getBody().path(path).toString());
  }
  ...
  public void getBodyContainsOr(String path, String path2, RequestSpecification rs, String body, String body2) {
    ...
    Assert.assertTrue(res.getBody().path(path).toString().replace(" ", "").toLowerCase().replace("[", "").replace("]", "") == body.replace(" ", "").toLowerCase().replace("[", "").replace("]", "") || res.getBody().path(path2).toString().replace(" ", "").toLowerCase().replace("[", "").replace("]", "") == body2.replace(" ", "").toLowerCase().replace("[", "").replace("]", ""));
    System.out.println("Response Body: " + res.getBody().path(path).toString());
